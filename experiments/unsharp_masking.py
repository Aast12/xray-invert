...
def run_optimization(target, hyperparams, operator=None, loss_fn=None, w0=None, **opt_params):
...
def optimize_unknown_processing(true_raw, target, operator,  run_init={}):
    ...
    loss_fn = ops.build_loss(ops.mse, ops.total_variation(run.config['total_variation']))

    recovered, loss = run_optimization(
        target, run.config, operator=operator, loss_fn=loss_fn, loss_logger=loss_logger, w0=w0
    )

    rec = recovered["image"]
    rec_range_normal = range_normalize(rec)
    utils.log_image('recovered', rec_range_normal)

    rec_processed = operator(recovered)
    utils.log_image('recovered_processed', range_normalize(rec_processed))
...
def setup_artifacts(raw_path, target_path, project=None):
    with wandb.init(
        project=project, job_type="data-preparation"
    ) as run:
...
if __name__ == "__main__":
    ...
    operator = ops.build_forward_fn(negative_log, unsharp_masking('low_'), unsharp_masking('high_'), windowing, range_normalize)
    ...
    wandb.agent(
        sweep_id,
        function=lambda: optimize_unknown_processing(true_raw, target, operator=operator, run_init=run_init),
